...
def all_participants_grater_than(months=6,when=['15-04-2024','15-04-2024','16-04-2024']):
    ...
    Used during 03/2024 to predict the number of RTSS doses that were gonna be
    given to ICARIA children next months. Deprecated.
    """
...
def rtss_counts():
    ...
    vaccinated with RTSS.
    :return: Gdrive with a summary of ICARIA participants vaccinated per Hf per dose.
    """
    ...
    for project_name in tokens.REDCAP_PROJECTS_ICARIA:
        print(project_name)
        project = redcap.Project(tokens.URL, tokens.REDCAP_PROJECTS_ICARIA[project_name])
        df = project.export_records(format_type='df',fields=['rtss_vacc_rtss1_date','rtss_vacc_rtss2_date','rtss_vacc_rtss3_date','rtss_vacc_rtss4_date'], event_name=['rtss_arm_1'])
        if not df.empty:
    ...
    total_first, total_second,total_third,total_four,total_all = 0, 0, 0, 0, 0
    ...
    ## If a file is needed apart from Gdrive: ##
